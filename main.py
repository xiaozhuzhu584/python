# -*- coding: utf-8 -*-
# ==============================================================================
# 学生出勤统计系统 - Flet 移动端兼容版
# ==============================================================================
# 适用平台：Android 平板/手机
# Python 版本：3.12+
# Flet 版本：0.23.0
# 功能：学生管理、出勤登记、统计报表、数据导出
# 特点：每次启动自动清除旧数据，移动端优化，无白屏问题
# ==============================================================================
...
def main_function(page_object: ft.Page):
    ...
    重要：移除所有 window 设置，适配移动端
    """
    # 设置应用程序窗口标题，显示在窗口标题栏
    page_object.title = "学生出勤统计"
    # 设置页面主题模式为浅色模式（LIGHT 或 DARK）
    page_object.theme_mode = ft.ThemeMode.LIGHT
    # 设置页面内边距为 20 像素，控件与窗口边缘的距离（移动端建议小一些）
    page_object.padding = 20
    # 设置页面滚动模式为自动滚动（移动端重要，允许内容超出时滚动）
    page_object.scroll = ft.ScrollMode.AUTO
    # 设置页面垂直对齐方式为从顶部开始
    page_object.vertical_alignment = ft.MainAxisAlignment.START
    # 设置页面水平对齐方式为中心对齐
    page_object.horizontal_alignment = ft.CrossAxisAlignment.CENTER

    # ⚠️ 重要：以下桌面端专用设置已删除（会导致移动端白屏）
    # page_object.window.width = 2400    # 已删除！移动端不支持
    # page_object.window.height = 1500   # 已删除！移动端不支持
    ...
    def show_snackbar_message(message_text):
        ...
        # 尝试执行消息显示操作
        try:
            # 创建 SnackBar 组件，包含要显示的文本消息（字体大小 16 像素）
            snackbar_control = ft.SnackBar(ft.Text(message_text, size=16))
    ...
    def open_student_manage_dialog(event):
        ...
        def refresh_student_list_display():
            ...
            for index_number, current_student_name in enumerate(student_name_list):
                ...
                student_list_column_control.controls.append(
                    ft.Row([
                        # 显示学生序号和姓名的文本控件（字体 18 像素）
                        ft.Text(f"{index_number + 1}. {student_name_for_delete}", size=18, expand=True),
                        # 删除按钮控件（红色背景，白色文字）
        ...
        student_manage_dialog = ft.AlertDialog(
            title=ft.Text("👥 学生名单管理", size=24),  # 设置弹窗标题，字体 24 像素
            content=ft.Column([
                # 输入框和添加按钮行控件
    ...
    def open_attendance_register_dialog(event):
        ...
        # 创建统计文本显示控件（字体 18 像素）
        attendance_stats_text_control = ft.Text("", size=18)
        ...
        def create_student_status_button_row(target_student_name):
            ...
            # 创建学生姓名和按钮组行控件
            return ft.Row([
                # 学生姓名文本控件（字体 18 像素，宽度 150 像素）
                ft.Text(target_student_name, size=18, width=150),
                # 四个状态按钮行控件（移动端简化为 emoji）
                ft.Row([
                    # 出勤按钮（绿色表示选中，灰色表示未选中）
                    ft.ElevatedButton("✅", color=ft.colors.WHITE,
                    ...
                    # 迟到按钮
                    ft.ElevatedButton("⚠️", color=ft.colors.WHITE,
                ...
                ], spacing=5),  # 按钮间距 5 像素（移动端紧凑布局）
            ], spacing=10)  # 姓名与按钮组间距 10 像素
        ...
        attendance_register_dialog = ft.AlertDialog(
            title=ft.Text("✅ 今日出勤登记", size=24),
            content=ft.Column([
                # 日期显示（字体 18 像素）
                ft.Text(f"日期：{today_date_string}", size=18),
                # 分隔线
                ft.Divider(),
                # 状态列表
    ...
    def open_statistics_report_dialog(event):
        ...
        for each_date_string in sorted(attendance_record_dict.keys(), reverse=True)[:30]:
            ...
            statistics_report_list_column.controls.append(
                ft.Container(
                    content=ft.Column([
                        # 日期标题（字体 20 像素，粗体）
                        ft.Text(f"📅 {each_date_string}", size=20, weight=ft.FontWeight.BOLD),
                        # 统计数据（字体 16 像素）
                        ft.Text(
        ...
        statistics_report_dialog = ft.AlertDialog(
            title=ft.Text("📊 出勤统计报表", size=24),
            content=ft.Container(content=statistics_report_list_column, width=600, height=600),
            actions=[ft.TextButton("❌ 关闭", on_click=close_dialog_function)],
        )
        # 打开弹窗
    ...
    def open_export_data_dialog(event):
        ...
        export_data_dialog = ft.AlertDialog(
            title=ft.Text("📤 导出数据", size=24),
            content=ft.Column([
                # 说明文字（字体 18 像素）
                ft.Text("数据已保存在 attendance_data.json", size=18),
                # 分隔线
                ft.Divider(),
                # 数据概览标题（字体 20 像素，粗体）
                ft.Text("📋 数据概览", size=20, weight=ft.FontWeight.BOLD),
                # 学生总数（字体 18 像素）
                ft.Text(f"学生总数：{len(student_name_list)}", size=18),
                # 记录天数（字体 18 像素）
                ft.Text(f"记录天数：{len(attendance_record_dict)}", size=18),
            ], spacing=15),
            actions=[ft.TextButton("❌ 关闭", on_click=close_dialog_function)],
        )
        # 打开弹窗
    ...
    page_object.add(
        ft.Column([
            # 应用标题（移动端字体 32 像素，蓝色，粗体，居中对齐）
            ft.Text("📚 学生出勤统计系统", size=32, weight=ft.FontWeight.BOLD, color=ft.colors.BLUE,
                    text_align=ft.TextAlign.CENTER),
            # 当前日期显示（字体 20 像素，灰色，居中对齐）
            ft.Text(f"日期：{get_current_date_string()}", size=20, color=ft.colors.GREY_700,
                    text_align=ft.TextAlign.CENTER),
            # 透明分隔线（高度 30 像素，增加间距）
            ...
            ft.Container(
                content=ft.Column([
                    # 卡片标题（字体 24 像素，粗体）
                    ft.Text("📋 今日出勤概览", size=24, weight=ft.FontWeight.BOLD),
                    # 提示信息（字体 18 像素，灰色）
                    ft.Text("每次启动自动清除旧数据", size=18, color=ft.colors.GREY),
        ...
        ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, expand=True)  # 主列居中对齐，占满空间
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        # view=ft.AppView.FLET_APP 使用 Flet 原生视图（移动端重要）
        # assets_dir="assets" 资源目录（如果有图片等资源文件）
        ft.app(
